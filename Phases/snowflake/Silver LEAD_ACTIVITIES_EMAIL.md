...
SELECT
    EXPECTED_ROWS,
    ACTUAL_ROWS,
    EXPECTED_ROWS - ACTUAL_ROWS AS DIFFERENCE
FROM expected
CROSS JOIN actual;
...
SELECT
    ...
    COUNT_IF(SUBJECT IS NOT NULL) AS WITH_SUBJECT,
    COUNT_IF(BODY_TEXT IS NOT NULL) AS WITH_BODY,
    COUNT_IF(SENDER IS NOT NULL) AS WITH_SENDER,
    COUNT_IF(OPENS_JSON IS NOT NULL) AS WITH_OPENS,
    COUNT_IF(HAS_REPLY = TRUE) AS WITH_REPLY
FROM SALES_ANALYTICS_DB.SILVER.LEAD_ACTIVITIES_EMAIL;
