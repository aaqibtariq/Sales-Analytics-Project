Step 1 — Validate Strategy outcomes
...
Step 3 — Validate Closer mapping
SELECT
    DEA_INTERNAL_NAME,
...
ORDER BY
    CALL_YEAR_MONTH,
    CLOSER_NAME;
3. Validate the report grain

There must be one row per closer and month:
...
0 rows
4. Validate total Strategy Calls
SELECT
...
7. Validate sales and financials
SELECT
    SUM(SALE) AS ATTRIBUTED_SALES,
    SUM(CASH_COLLECTED) AS ATTRIBUTED_CASH_COLLECTED
FROM GOLD.CLOSER_REPORT;

The attributed sales may be lower than the global 270 sales because the requirement allows a sale only when it follows a documented attended Strategy Call. The view intentionally excludes sales that cannot be safely attributed to a prior attended call and closer.

8. Validate no impossible results
SELECT *
FROM GOLD.CLOSER_REPORT
